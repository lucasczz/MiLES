...
class GeolifeMovesModule(L.LightningDataModule):
    def collate_fn(batch):
        moves, y = zip(*batch)
        return moves, torch.tensor(y)

    def __init__(
        self, batch_size=128, val_split=0.1, test_split=0.2, n_hex_rows=50, n_workers=4
    ):
        super().__init__()
        ...
        self.trajectories = {}
        with open(DATAPATH.joinpath(f"geolife_hex_{n_hex_rows}_moves.pkl"), "rb") as f:
            self.df = pickle.load(f)

    def get_info(self):
        return {
            "n_users": self.df["user"].max().item() + 1,
    ...
    def setup(self, stage):
        ...
        for _, df_user in self.df.groupby("user"):
            ...
            n_train = int(n_trajectories * (1 - self.val_split - self.test_split))
            n_val = int(n_trajectories * self.val_split)
        ...
        for stage in stages:
            ...
            moves = [torch.tensor(traj["move"].values) for _, traj in trajs]
            y = torch.tensor([traj.iloc[0]["user"] for _, traj in trajs])
            self.trajectories[stage] = list(zip(moves, y))
    ...
    def train_dataloader(self):
        return data.DataLoader(
            ...
            shuffle=True,
            collate_fn=GeolifeMovesModule.collate_fn,
